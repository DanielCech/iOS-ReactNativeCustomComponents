...
open class BottomContainerView: UIView {
    ...
    open lazy var retakeButton: UIButton = { [unowned self] in
        let button = UIButton()
        button.setTitle(self.configuration.retakeButtonTitle, for: UIControlState())
        button.titleLabel?.font = self.configuration.doneButton
        button.addTarget(self, action: #selector(retakeButtonDidPress(_:)), for: .touchUpInside)
    ...
    func configure() {
        [borderPickerButton, pickerButton, doneButton, stackView, topSeparator, retakeButton].forEach {
            addSubview($0)
    ...
    @objc func retakeButtonDidPress(_ button: UIButton) {
    ...
    func setPickerButtonHidden(_ hidden: Bool) {
        pickerButton.hideWithFade(hidden)
        borderPickerButton.hideWithFade(hidden)
    }
    
    func setRetakeButtonHidden(_ hidden: Bool) {
        retakeButton.hideWithFade(hidden)
    }
    
    func setDoneButtonEnabled(_ enabled: Bool) {
        doneButton.setTitle(enabled ? "Accept" : "Cancel", for: .normal)
    }
