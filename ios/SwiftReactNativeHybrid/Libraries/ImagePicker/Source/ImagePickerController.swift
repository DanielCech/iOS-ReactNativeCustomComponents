...
open class ImagePickerController: UIViewController {
    ...
    fileprivate var _focusOverlayView: FocusOverlayView?
    fileprivate var _patientFocusOverlayView: PatientFocusOverlayView?
    
    var scanFlow: ScanFlow?
    var overlayFrame: CGRect?
    var croppedImage: UIImage?
    
    var controllerState: ImagePickerState = .takingPhoto {
        didSet {
            switch controllerState {
            case .takingPhoto:
                _focusOverlayView?.hideWithFade(false)
                _patientFocusOverlayView?.hideWithFade(false)
                bottomContainer.setPickerButtonHidden(false)
                bottomContainer.setRetakeButtonHidden(true)
                bottomContainer.setDoneButtonEnabled(false)
                topView.hideWithFade(false)
                bottomContainer.pickerButton.isEnabled = true
                cameraController.view.hideWithFade(false)
                croppedImageView.hideWithFade(true)
                
            case .confirmCropping:
                _focusOverlayView?.hideWithFade(true)
                _patientFocusOverlayView?.hideWithFade(true)
                bottomContainer.setPickerButtonHidden(true)
                bottomContainer.setRetakeButtonHidden(false)
                bottomContainer.setDoneButtonEnabled(true)
                topView.hideWithFade(true)
                cameraController.view.hideWithFade(true)
                croppedImageView.hideWithFade(false)
            }
        }
    }
    ...
    open lazy var croppedImageView: UIImageView = {
    ...
    open override func viewDidLoad() {
        ...
        for subview in [cameraController.view, croppedImageView, bottomContainer, topView] {
        ...
        if let referenceView: UIView = _focusOverlayView ?? _patientFocusOverlayView {
            croppedImageView.leftAnchor.constraint(equalTo: referenceView.leftAnchor).isActive = true
            croppedImageView.rightAnchor.constraint(equalTo: referenceView.rightAnchor).isActive = true
            croppedImageView.topAnchor.constraint(equalTo: referenceView.topAnchor).isActive = true
            croppedImageView.bottomAnchor.constraint(equalTo: referenceView.bottomAnchor).isActive = true
        }
    }
...
//        UIApplication.shared.setStatusBarHidden(true, with: .fade)
    ...
    open override func viewDidAppear(_ animated: Bool) {
        ...
        guard let unwrappedScanFlow = scanFlow else { return }
        
        if unwrappedScanFlow == .patientPhoto {
            overlayFrame = _patientFocusOverlayView!.frame
        }
        else {
            overlayFrame = _focusOverlayView!.frame
        }
    }
...
//        controllerState = .confirmCropping
//        let rotatedImage = ImageUtils.correctOrientationOf(image: image)
//        croppedImage = rotatedImage
//        croppedImageView.image = rotatedImage
        
        
        let orientedImage: UIImage
        if image.size.width > image.size.height {
            orientedImage = UIImage(cgImage: image.cgImage!, scale: 1, orientation: .right)
        }
        else {
            orientedImage = image
        }
        ...
        croppedImageView.image = orientedImage
    }
...
extension ImagePickerController: CameraViewDelegate {
    ...
    func cameraDidTakePhoto(image: UIImage) {
        delegate?.imagePicker(pickerController: self, didTakePhoto: image)
    }
    ...
    func imageToLibrary() {
        // We don't need to store images in library
        return
    ...
    @objc public func handleRotation(_ note: Notification) {
        ...
        UIView.animate(withDuration: 0.25, animations: {
            [self.topView.rotateCamera, self.bottomContainer.pickerButton,
             self.bottomContainer.stackView, self.bottomContainer.doneButton, self.bottomContainer.retakeButton].forEach {
                $0.transform = rotate
            }
...
extension ImagePickerController {
    func setupFocusView() {
        ...
        if unwrappedScanFlow == .patientPhoto {
            _patientFocusOverlayView = PatientFocusOverlayView(frame: .zero)
            _patientFocusOverlayView?.translatesAutoresizingMaskIntoConstraints = false
            _patientFocusOverlayView?.isUserInteractionEnabled = false
            view.addSubview(_patientFocusOverlayView!)
        }
        else {
            _focusOverlayView = FocusOverlayView(frame: .zero)
            _focusOverlayView?.translatesAutoresizingMaskIntoConstraints = false
            _focusOverlayView?.isUserInteractionEnabled = false
            view.addSubview(_focusOverlayView!)
        }
        
        switch unwrappedScanFlow {
        case .patientPhoto:
            _patientFocusOverlayView?.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
            _patientFocusOverlayView?.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: -DCToolbox.screenHeight() / 12.0).isActive = true
            _patientFocusOverlayView?.leftAnchor.constraint(equalTo: view.leftAnchor, constant: DCToolbox.screenWidth() / 10.0).isActive = true
            _patientFocusOverlayView?.widthAnchor.constraint(equalTo: _patientFocusOverlayView!.heightAnchor, multiplier: 1).isActive = true
            
        case .idCard:
            _focusOverlayView?.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
            _focusOverlayView?.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: -DCToolbox.screenHeight() / 16.0).isActive = true
            _focusOverlayView?.leftAnchor.constraint(equalTo: view.leftAnchor, constant: DCToolbox.screenWidth() / 8.0).isActive = true
            _focusOverlayView?.widthAnchor.constraint(equalTo: _focusOverlayView!.heightAnchor, multiplier: 0.62).isActive = true
            
        case .doctorReferenceLetter:
            _focusOverlayView?.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
            _focusOverlayView?.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: -DCToolbox.screenHeight() / 14.0).isActive = true
            _focusOverlayView?.leftAnchor.constraint(equalTo: view.leftAnchor, constant: DCToolbox.screenWidth() / 10.0).isActive = true
            _focusOverlayView?.widthAnchor.constraint(equalTo: _focusOverlayView!.heightAnchor, multiplier: 0.7).isActive = true
        }
    }
}
